...
from models.distnet import NaiveNet
...
# Checkpoints
parser.add_argument('--save', '-s', type=str, default='./snapshots/baseline1', help='Folder to save checkpoints.')
    ...
    with experiment.test():
        ...
        with torch.no_grad():
            for data, target in test_loader:
                ...
                # forward
                output = net(data)
                loss = F.cross_entropy(output, target).mean() - 0.1 * torch.gather(output, 1, target.view(-1, 1)).mean()
